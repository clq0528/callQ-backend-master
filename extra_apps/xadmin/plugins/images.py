...
class AdminImageWidget(forms.FileInput):
    ...
    def render(self, name, value, attrs=None, renderer=None):
        ...
        if value and hasattr(value, "url"):
            ...
            #sftp图片显示
            # output.append('<a href="%s" target="_blank" title="%s" data-gallery="gallery"><img src="%s" class="field_img"/></a><br/>%s ' %
            #     (value.url.replace('/media', 'http://192.168.56.77:8800'), label,value.url.replace('/media', 'http://192.168.56.77:8800'), _('Change:')))
    ...
    def get_field_result(self, result, field_name):
        if isinstance(result.field, models.ImageField):
            if result.value:
                ...
                #sftp图片显示
                # result.text = mark_safe('<a href="%s" target="_blank" title="%s" data-gallery="gallery"><img src="%s" class="field_img"/></a>' %
                #                         (img.url.replace('/media','http://192.168.56.77:8800'), result.label, img.url.replace('/media','http://192.168.56.77:8800')))
                self.include_image = True
        return result
